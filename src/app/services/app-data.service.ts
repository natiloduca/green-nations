...
export class AppDataService {
	...
	private countries: Array<Country> = [
		...
		{ id: 2, name: "Luxemborug", epiIndex: 83.29 },
		{ id: 3, name: "Australia", epiIndex: 82.4 },
		{ id: 4, name: "Singapure", epiIndex: 81.78 },
	...
	createCountry(vm: Country) : Observable<any> {
		//return  Observable.of({}).delay(2000).flatMap(x=>Observable.throw('Create error'));
		let id = 0;
	...
	updateCountry(updatedCountry: Country) : Observable<any> {
		return  Observable.of({}).delay(2000).flatMap(x=>Observable.throw('update error'));
	}
}